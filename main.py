...
class Main(object):
	...
	def load_data(self):
		...
		for split in ['train', 'valid']:		#只看训练和验证集
		...
		self.rel2id.update({rel+'_reverse': idx+len(self.rel2id) for idx, rel in enumerate(rel_set)})  #rel2id也添加反向关系

		self.id2ent = {idx: ent for ent, idx in self.ent2id.items()}
		self.id2rel = {idx: rel for rel, idx in self.rel2id.items()}

		self.p.num_ent		= len(self.ent2id)		#实体数量
		self.p.num_rel		= len(self.rel2id) // 2		#关系数量
		self.p.embed_dim	= self.p.k_w * self.p.k_h if self.p.embed_dim is None else self.p.embed_dim  # 10*20=200

		self.data	= ddict(list)	#三个数据集分别以数字三元组形式放到data分类中
		sr2o		= ddict(set)	#将训练集中的三元组以 (sub, rel)-->obj 形式存储

		for split in ['train', 'test', 'valid']:	#训练，验证，测试集
			for line in open('./data/{}/{}.txt'.format(self.p.dataset, split)):
				sub, rel, obj = map(str.upper, line.strip().split('\t'))
				sub, rel, obj = self.ent2id[sub], self.rel2id[rel], self.ent2id[obj]	#都转换为索引表示
				self.data[split].append((sub, rel, obj))	#三个数据集分别以数字三元组形式放到data分类中

				if split == 'train': 	#训练集，找出某节点某关系下的邻居集合
					sr2o[(sub, rel)].add(obj)
					sr2o[(obj, rel+self.p.num_rel)].add(sub)	#注意反向关系
		self.data = dict(self.data)

		self.sr2o = {k: list(v) for k, v in sr2o.items()}	#只包含训练集的邻居集合（注意带不带sel字样）
		for split in ['test', 'valid']:		#验证集和测试集，找出某节点某关系下的邻居集合
			for sub, rel, obj in self.data[split]:
				sr2o[(sub, rel)].add(obj)
				sr2o[(obj, rel+self.p.num_rel)].add(sub)

		self.sr2o_all = {k: list(v) for k, v in sr2o.items()}	#所有数据集的邻居集合

		self.triples = ddict(list)

		for (sub, rel), obj in self.sr2o.items():		#训练集 放到triples，邻居列表作为label
			self.triples['train'].append({'triple':(sub, rel, -1), 'label': self.sr2o[(sub, rel)], 'sub_samp': 1})

		for split in ['test', 'valid']:		#验证、测试集放到triples，邻居列表作为label
			for sub, rel, obj in self.data[split]:
				rel_inv = rel + self.p.num_rel			#注意区分 tail 和 head，表示给定头和关系预测尾/给定为和关系预测头
		...
		def get_data_loader(dataset_class, split, batch_size, shuffle=True):
			return  DataLoader(
					...
					collate_fn      = dataset_class.collate_fn		#得到三元组和向量label
				)
	...
	def construct_adj(self):	#构建连街边、连接类型
	...
	def get_chequer_perm(self):		#InteractE 重塑
		# p.perm 特征重排数量,默认为1
		ent_perm  = np.int32([np.random.permutation(self.p.embed_dim) for _ in range(self.p.perm)])		#随机排列，例如将0-200随机排列
	...
	def save_model(self, save_path):
		...
		torch.save(state, save_path+'my_model.pth')
	...
	def load_model(self, load_path):
		...
		self.model.load_state_dict(state_dict)		#用于将预训练的参数权重加载到新的模型之中
	...
	def run_epoch(self, epoch):	#一次epoch中包含多batch
		self.model.train()
		losses = []
		...
		loss = np.mean(losses)		#一个epoch的平均loss
	...
	def fit(self):
		...
		if self.p.restore:
			self.load_model(save_path+'my_model.pth')
			self.logger.info('Successfully Loaded previous model')

		for epoch in range(self.p.max_epochs):	#500 epoch
			train_loss	= self.run_epoch(epoch)		#计算训练损失-->然后验证集上评估
...
if __name__ == "__main__":
	...
	parser.add_argument('-num_bases',	dest='num_bases', 	default=-1,   	type=int, 	help='Number of basis relation vectors to use')	#基关系向量的个数，默认无该向量
