...
class TrainDataset(Dataset):
	def __init__(self, triples, params):
		...
		self.p 		= params
		self.entities	= np.arange(self.p.num_ent, dtype=np.int32)		#实体数量
	...
	def collate_fn(data):
		triple		= torch.stack([_[0] 	for _ in data], dim=0)	#所有的三元组
		trp_label	= torch.stack([_[1] 	for _ in data], dim=0)	#所有label，每个label以01向量表示
	...
	def get_label(self, label):		#一个向量，是邻居的标1，否则为0
